...
def render_sleep_analytics(dark_mode: bool, normalized: dict):
        ...
        sleep_df = find_df_by_datatype(normalized, "sleep_observation") or find_df_by_datatype(normalized, "com.samsung.shealth.sleep")
        sleep_stage_df = find_df_by_datatype(normalized, "sleep_stage_observation") or find_df_by_datatype(normalized, "com.samsung.health.sleep_stage")
        
        if sleep_df is None or sleep_df.empty:
            st.warning("📊 No sleep summary data found. Please ensure your health export or vault connection contains sleep data.")
        else:
