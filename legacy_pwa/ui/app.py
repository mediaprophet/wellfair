...
def main():
    ...
    vault_state = fetch_vault_state()
    
    if not vault_state or not vault_state.get('linked'):
        if not _is_demo_profile:
            st.warning("🔗 **Waiting for Mobile Vault link...**")
            st.info("Please open your mobile vault and pair with the desktop connector.")
            if st.button("Refresh Link Status"):
                st.rerun()
            st.stop()
        else:
            # Developer override for testing layout
        ...
        st.success(f"✅ **Mobile Vault Linked** (Synced: {vault_state.get('last_sync', 'Live')})")
        normalized = vault_state.get('data', {})
